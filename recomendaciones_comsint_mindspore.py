...
class Recomendador():
    ...
    def __init__(self,
        ...
        # Verificamos si hay dispositivos GPU disponibles
        if context.get_context('device_target') == 'GPU':
            try:
                # Establecemos el límite de memoria para la GPU
                context.set_auto_parallel_context(device_memory_capacity=1024)
            except:
                print('No se encontró hardware GPU')
                # Desactivamos la visibilidad de dispositivos GPU
                context.set_visible_devices([], 'GPU')
    ...
    def GenerarModeloRegresionCNN(self, input_shape, emb_size, numero_salidas, kernels=128):
            ...
            Utiliza la clase ModeloCNNNut para generar un modelo con MindSpore
    ...
    def EvaluarModeloRegresion(self, INFO_COLS, history, x_val, y_val, modelo=None):
        ...
        @history: Una instancia de TrainLineage de Mindspore
        ...
        try:
            pd.DataFrame(history.to_dict())
            plt.show()
        except:
            pass


        # Crear el conjunto de validación como un objeto Dataset
        val_dataset = ds.create_dataset(x_val, y_val)

        # Crear el objeto métrica para el cálculo de la exactitud
        metric = metrics.Accuracy()

        # Cálculo de la exactitud y pérdida para el conjunto de validación
        for data, label in val_dataset.create_dict_iterator():
            output = modelo(Tensor(data))
            metric.update(output, Tensor(label))
        val_acc = metric.eval()
        val_loss = metrics.loss_fn(output, Tensor(label))
        print('Val Accuracy:', val_acc, ' Val_loss:',val_loss )

        input_tensor = Tensor(x_val, mindspore.float32)
        test_predictions = modelo.predict(input_tensor)
    ...
    def EntrenarModelo(self, df_nutricionales='nutricion.csv', 
        ...
        @steps_per_epoch: El número de pasos de entrenamiento de cada época
        @device_target: El tipo de procesador para entrenar (NPU para Ascend, default=CPU)
        ...
        # device target = NPU si estamos en Ascend
        context.set_context(mode=context.GRAPH_MODE, device_target=device_target) 


        # El modelo se guardará en la carpeta basedir + 'Modelos/'
        archivoC = self.basedir + 'Modelos/Mindspore/Modelo_Nut_FV_DistilBERT_0'+str(version)+'_EMBED-'+ str(self.EMB_SIZE) +'_CNN_MINDSPORE.ckpt'

        check_fileC = os.path.isfile(archivoC)

        if check_fileC:
            self.modeloCNN = load_checkpoint(archivoC)
        ...
        for epoch in range(epochs):
            ...
            val_loss, val_mae = metrics.model_eval(test_dataset)
            print('Epoch: {}, Loss: {}, Val_loss: {}, Val_mae: {}'.format(epoch, loss, val_loss, val_mae))

        if (save): 
            # Guardar el modelo
            save_checkpoint(self.modeloCNN, archivoC)
            print('modelo guardado en:', archivoC)

        # Obtener objeto TrainLineage del modelo cargado
        history = self.modeloCNN.get_train_lineage()
